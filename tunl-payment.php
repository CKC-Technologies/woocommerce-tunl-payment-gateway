 ...
 * Plugin Name: Tunl Payment For WooCommerce

 * Plugin URI: https://www.brihaspatitech.com/

 * Description: A plugin is used to integrate the Tunl with your eCommerce store.
...
function initialize_tunl_class() {
    ...
    class WC_TUNL_Gateway extends WC_Payment_Gateway {
        ...
        public function __construct() {
		    ...
		    $this->title = __( 'Tunl Payment', 'tunlwoopay' );

		    $this->method_title = __( 'Tunl Payment', 'tunlwoopay' );

		    $this->method_description = __( 'Tunl payment gateway', 'tunlwoopay' );
		...
		public function init_form_fields(){
			...
			$array_fields['enabled'] = array(
	            ...
	            'label'       => __( 'Enable Tunl Payment', 'tunlwoopay' ),
			...
			$array_fields['title'] = array(
				...
				'default'     => __( 'Tunl Payment', 'tunlwoopay' ),
	        ...
	        $array_fields['username'] = array(
				...
				'title'       => __( 'Username', 'tunlwoopay' ),

				'label'       => __( 'Username', 'tunlwoopay' ),
			...
			$array_fields['password'] = array(
				...
				'title'       => __( 'Password', 'tunlwoopay' ),

				'label'       => __( 'Password', 'tunlwoopay' ),
		...
		public function payment_fields() {	  
		    ...
		    <fieldset id="wc-<?php echo esc_attr( $this->id ); ?>-cc-form" class="wc-credit-card-form wc-payment-form" style="background:transparent;">
		        ...
		        <?php echo sprintf(
						'<div class="form-row form-row-wide">%s</div>',
						wp_kses(
							sprintf(
				...
				<?php echo sprintf(
						...
						wp_kses(
							sprintf(
								__( '<label>Expiry Date <span class="required">*</span></label>
				...
				<?php echo sprintf(
						...
						wp_kses(
							sprintf(
								__( '<label>Card Code <span class="required">*</span></label>
									<input id="tunl_cvc" type="password" autocomplete="off" placeholder="CVC" class="input-text" name="tunl_cardcode">' )
							),
							array(
								'label' => true,
								'span' => array(
		...
		public function process_payment( $order_id ) {
			...
			if( empty( $my_options['api_mode'] ) || ( $my_options['api_mode'] == 'no' ) ){
				...
				$url = TUNL_LIVE_URL.'/merchants/'.$my_options['tunl_merchantId'].'/contacts';

			}else{

				$url = TUNL_TEST_URL.'/merchants/'.$my_options['tunl_merchantId'].'/contacts';
			...
			$resut_data_check = array_column($resut_data,'email');

			$check_key = array_search($email,$resut_data_check);

			/**  Check contact/customer detail */
			
			if( !empty( $check_key ) || ( $check_key === 0 ) ){

				/**  Check contact/customer detail if already exits */

				$contactId = $resut_data[$check_key]['id'];

			}else{

				/** create new contact/customer user with tunl payment api */
				...
				$contactId = $resut_data['id'];

			}

			/** create new account for contact/customer user with tunl payment api */

			if( empty( $my_options['api_mode'] ) || ( $my_options['api_mode'] == 'no' ) ){

				$url = TUNL_LIVE_URL.'/merchants/'.$my_options['tunl_merchantId'].'/contacts/'.$contactId.'/accounts';

			}else{

				$url = TUNL_TEST_URL.'/merchants/'.$my_options['tunl_merchantId'].'/contacts/'.$contactId.'/accounts';
	...
	if( $data['payment_method'] == 'tunl' ){
		...
		if( empty( $my_options['connect_button'] ) || ( $my_options['connect_button'] == 1 ) ){
			...
			if( empty($_POST['tunl_expirydate']) ){
				...
				$errors->add( 'validation', '<strong>Expiry Date</strong> is a required field.' );
	...
	if( !empty($my_options) ){
		if( ( empty( $my_options['connect_button'] ) || ( $my_options['connect_button'] == 1 ) ) && ( $my_options['api_mode'] == 'yes' ) ){
			...
			echo '<div class="notice notice-warning is-dismissible">
					  <p>Tunl Payment merchant credentials are not verified. You can authenticate by entering information.</p>
				  </div>'; 
...
function tunl_auth_error_action() {
	...
	echo '<div class="notice notice-error is-dismissible">
	      	<p>Tunl Payment merchant credentials seem to be wrong. Please enter correct information.</p>
	      </div>';
...
function woocommerce_tunl_missing_wc_notice() {
	...
	echo '<div class="error"><p><strong>' . sprintf( esc_html__( 'Tunl Payment requires WooCommerce to be installed and active. You can download %s here.', 'woocommerce-gateway-stripe' ), '<a href="https://woocommerce.com/" target="_blank">WooCommerce</a>' ) . '</strong></p></div>';
